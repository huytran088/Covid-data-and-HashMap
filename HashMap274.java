...
public class HashMap274<K, V> implements Map<K, V> {
	...
	public V put(K key, V value) {
		...
		for (int i = 0; i < buckets[hashIndex].size(); i++) {
			if (buckets[hashIndex].get(i).key.equals(key)) {
				V oldValue = buckets[hashIndex].get(i).value;
				buckets[hashIndex].get(i).value = value;
				return oldValue;
			}
		}
	...
	public V get(K key) {
		...
		for (int i = 0; i < buckets[hashIndex].size(); i++) {
			if (buckets[hashIndex].get(i).key.equals(key)) {
				return buckets[hashIndex].get(i).value;
			}
		}
		return null;
	}
	...
	public V remove(K key) {
		...
		for (int i = 0; i < buckets[hashIndex].size(); i++) {
			if (buckets[hashIndex].get(i).key.equals(key)) {
				V value = buckets[hashIndex].get(i).value;
